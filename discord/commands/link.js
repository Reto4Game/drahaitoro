...
module.exports.run = async (client, message, args) => {
    ...
    if (!args[2] || !message.mentions.users.first()) return createEmbed('info', 'Link Command', `Command to link discord users to their minecraft account.` + "\n\n**Usage**\n\n" +  "``!link <discord tag> <minecraft username>``\n\nTo see a list of available capes, use ``!list cape``", null, message)
    let member = message.mentions.users.first();
    users.setLink(member.id, args[2], output => {
      if (output === true) {
